...
## Approach
I will be using the `dplyr` package in R, specifically focusing on grouping and windowing operations. My strategy involves three main steps. First I will use `group_by(Instrument)` to ensure that the mathematical "windows" are isolated. This prevents the price data from Bitcoin from bleeding into the calculations for Ethereum. 
...
# We use kable to make the output look professional in your HTML
knitr::kable(head(final_analysis, 15), 
